<?php
function evita_breadcrumbs() {
	
	$showOnHome	= esc_html__('1','evita'); 	// 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter 	= '';   // Delimiter between breadcrumb
	$home 		= esc_html__('Home','evita'); 	// Text for the 'Home' link
	$showCurrent= esc_html__('1','evita'); // Current post/page title in breadcrumb in use 1, Use 0 for don't show
	$before		= '<li class="active"><span>'; // Tag before the current Breadcrumb
	$after 		= '</span></li>'; // Tag after the current Breadcrumb
	...
	if ($showOnHome == 1) echo '<li><a href="' . esc_url($homeLink) . '"><i class="fa fa-home"></i>  ' . esc_html($home) . '</a>Blog</li>';
 
	} else {
 
    echo '<li><a href="' . esc_url($homeLink) . '"><i class="fa fa-home"></i>  ' . esc_html($home) . '</a> ' . '&nbsp' . wp_kses_post($breadcrumb_seprator) . '&nbsp';
 
    if ( is_category() ) 
	{
		$thisCat = get_category(get_query_var('cat'), false);
		if ($thisCat->parent != 0) echo get_category_parents($thisCat->parent, TRUE, ' ' . ' ');
		echo $before . esc_html__('Archive by category','evita').' "' . esc_html(single_cat_title('', false)) . '"' .$after;
		
	} 
	
	elseif ( is_search() ) 
	{
		echo $before . esc_html__('Search results for ','evita').' "' . esc_html(get_search_query()) . '"' . $after;
	} 
	...
	elseif ( is_page() && !$post->post_parent ) 
	{
		...
		if ($showCurrent == 1) echo $before . esc_html(wp_title('')) . $after;
	} 
	...
	elseif ( is_tag() ) 
	{
		echo $before . esc_html__('Posts tagged ','evita').' "' . esc_html(single_tag_title('', false)) . '"' . $after;
	} 
	
	elseif ( is_author() ) {
		global $author;
		$userdata = get_userdata($author);
		echo $before . esc_html__('Articles posted by ','evita').'' . $userdata->display_name . $after;
	} 
	
	elseif ( is_404() ) {
		echo $before . esc_html__('Error 404 ','evita'). $after;
    }
	
    if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
		echo ' ( ' . esc_html__('Page','evita') . '' . esc_html(get_query_var('paged')). ' )';
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
