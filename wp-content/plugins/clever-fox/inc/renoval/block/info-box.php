...
function renoval_block_categories( $categories ) {
...
if ( version_compare( $GLOBALS['wp_version'], '5.8-alpha-1', '<' ) ) {
    add_filter( 'block_categories', 'renoval_block_categories', 10, 2 );
} else {
    add_filter( 'block_categories_all', 'renoval_block_categories', 10, 2 );
}

// Register Blocks
function renoval_wp_register_script( $block, $options = array() ) {
    register_block_type( 'info-box/' . $block,
        array_merge( array(
            'editor_script' => 'renoval_editor_script',
            'editor_style'  => 'renoval_editor_style',
            'script'        => 'renoval_script',
            'style'         => 'renoval_style',
        ), $options )
    );
}
...
// Enqueue assets
function renoval_enqueue_block_assets() {
    wp_enqueue_script( 'font-awesome-kit', CLEVERFOX_PLUGIN_URL . '/inc/renoval/block/assets/js/font-awesome-kit.js', array(), '1.0', true );
}
add_action( 'enqueue_block_assets', 'renoval_enqueue_block_assets' );

function renoval_register() {
    // Resister script in editor
	 wp_register_script( 'renoval_editor_script', CLEVERFOX_PLUGIN_URL . '/inc/renoval/block/dist/editor.js', array( 'wp-blob', 'wp-block-editor', 'wp-blocks', 'wp-components', 'wp-compose', 'wp-data', 'wp-element', 'wp-html-entities', 'wp-i18n', 'wp-rich-text', 'font-awesome-kit' ), '1.0', false ); 
    wp_register_style( 'renoval_editor_style', CLEVERFOX_PLUGIN_URL . '/inc/renoval/block/dist/editor.css', array( 'wp-edit-blocks'), null );

    // Register Blocks
    renoval_wp_register_script( 'infos', array( 'render_callback' => 'render_renoval_infos' ) );
    renoval_wp_register_script( 'info', array( 'render_callback' => 'render_renoval_info' ) );
}
add_action( 'init', 'renoval_register' );

// Generate Styles
class RenovalInfoStyleGenerator {
...
// Render Infos
function render_renoval_infos($attributes, $content){
    extract( $attributes );

    $cId = $cId ?? '';

    // Generate Styles
    $infosStyle = new RenovalInfoStyleGenerator();
...
// Render Info
function render_renoval_info( $attributes ) {
    extract( $attributes );
    ...
    // Generate Styles
    $infoStyle = new RenovalInfoStyleGenerator();
    $infoStyle::addStyle("#renovalInfo-$cId .renovalInfo .icon", array(
    ...
    ob_start(); // Echo the content
    // echo "<div class='av-column-".$columns['desktop']." ".$infoTypes."' id='renovalInfo-$cId'>
