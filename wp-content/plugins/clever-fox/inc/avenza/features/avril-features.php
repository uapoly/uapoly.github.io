<?php
function cleverfox_avril_features_setting( $wp_customize ) {
$selective_refresh = isset( $wp_customize->selective_refresh ) ? 'postMessage' : 'refresh';
	...
	$wp_customize->add_setting( 
    	'features_bg_setting' , 
    	array(
			'default' 			=> CLEVERFOX_PLUGIN_URL .'inc/avenza/images/feature-img.png',
			'capability'     	=> 'edit_theme_options',
	...
	$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize , 'features_bg_setting' ,
		array(
	 ...
	 */
		...
		//Pro feature
		class cleverfox_Avril_feature__section_upgrade extends WP_Customize_Control {
		...
		$wp_customize->add_control(
			new cleverfox_Avril_feature__section_upgrade(
			$wp_customize,
			'cleverfox_avril_feature_upgrade_to_pro',
				array(
					'section'				=> 'feature_setting',
...
add_action( 'customize_register', 'cleverfox_avril_features_setting' );

// service selective refresh
function cleverfox_avril_features_section_partials( $wp_customize ){	
	// feature_title
	$wp_customize->selective_refresh->add_partial( 'feature_title', array(
		...
		'render_callback'  => 'cleverfox_avril_feature_title_render_callback',
	...
	$wp_customize->selective_refresh->add_partial( 'feature_subtitle', array(
		...
		'render_callback'  => 'cleverfox_avril_feature_subtitle_render_callback',
	...
	$wp_customize->selective_refresh->add_partial( 'feature_description', array(
		...
		'render_callback'  => 'cleverfox_avril_feature_description_render_callback',
...
add_action( 'customize_register', 'cleverfox_avril_features_section_partials' );

// feature_title
function cleverfox_avril_feature_title_render_callback() {
	return get_theme_mod( 'feature_title' );
}

// feature_subtitle
function cleverfox_avril_feature_subtitle_render_callback() {
	return get_theme_mod( 'feature_subtitle' );
}

// service description
function cleverfox_avril_feature_description_render_callback() {
